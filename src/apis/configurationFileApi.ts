...
export const configurationFileApiHandlerBuilder = (fastifyInstance: DecoratedFastify): Handler<any> => {
  ...
  return async(request, reply) => {
    if (instanceConfig.PLUGINS_CONFIGURATIONS_PATH) {
      ...
      const fileContent = await retrieveFunction(instanceConfig, configurationName, request.getGroups(), getPermissions(request, userPropertiesHeader as string))
